...
def users(usertype):
    ...
    if usertype == 'students':
        users = User.query.filter_by(role='Student').order_by(User.firstname.asc()).paginate(page, 50, False)
        if users:     
        ...
        users = User.query.filter(User.role!='Student').order_by(User.firstname.asc()).paginate(page, 50, False)           
        if users:     

...
def filter(category):
    if request.method == 'POST':
        ...
        if search == 'Balanced':
            ...
            dt = datetime.strptime(search, '%Y-%m-%d %H:%M:%S')
            if category == 'prospects':
                ...
                clients = Prospects.query.filter(or_(Prospects.fullname.ilike(f'%{search}%'), Prospects.email.ilike(f'%{search}%'), Prospects.phone.ilike(f'%{search}%'), Prospects.location.ilike(f'%{search}%'), Prospects.data_source.ilike(f'%{search}%'), Prospects.sector.ilike(f'%{search}%'), Prospects.company_name.ilike(f'%{search}%'), Prospects.courses.ilike(f'%{search}%'), Prospects.status.ilike(f'%{search}%'), Prospects.remark.ilike(f'%{search}%'), Prospects.extra1.ilike(f'%{search}%'), Prospects.extra2.ilike(f'%{search}%'), Prospects.extra3.ilike(f'%{search}%'), Prospects.date_created == (f'%{dt}%'))).all()
            elif category == 'students':
